...
const AddProduct = (props) => {
  ...
  useEffect(() => {
    const getProduct = async () => {
      try {
        const response = await axios.get(`/product/products/${props.id}`);
        const { data } = response;

        setProduct(data.product);
        setCategoriesId(data.product.category);
        setTagsId(data.product.tag);
        setAddedPhotos(data.product.images);
